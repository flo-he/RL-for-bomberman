...
def learn_from_exp(self):
    ...
    '''   
    # get the batch
    states, actions, rewards, next_states, continues = self.batch_queue.get()

    # online dqn computes the q values for the next state and determines best action
    with self.DQNLock:
        #print("learn acquired DQNLock")
        next_state_q, target_q = self.sess.run(
                                    [self.online_dqn.output_layer, self.target_dqn.output_layer],
                                    feed_dict={
                                        self.online_dqn.input_layer: next_states,
                                        self.target_dqn.input_layer: next_states
                                    }
                                )
        ...
        y_val = rewards.reshape(-1, 1) + continues.reshape(-1, 1) * self.discount_rate * target_q[dim_0_indices, best_action].reshape(-1, 1)

        # actual training step
        loss, _ , __= self.sess.run(
                    [self.online_dqn.loss, self.online_dqn.training_op, self.copy_online_to_target],
                    feed_dict = {
                        self.online_dqn.input_layer: states,
                        self.online_dqn.actions_input: actions,
                        self.online_dqn.y_Q: y_val
                    }
                )
